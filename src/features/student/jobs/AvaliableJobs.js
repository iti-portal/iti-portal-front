...
// The JobCard component with visual tweaks to match the new design
    ...
    <div
      className={`border rounded-lg p-4 cursor-pointer transition-all duration-200 relative 
        ${isSelected
          ...
          : 'bg-white border-gray-200 hover:border-[#901b20]/50 hover:bg-red-50/50 hover:shadow-sm'}`}
...
// The main component with the new design applied
export default function AvailableJobs() {
    ...
    useEffect(() => {
        ...
        const filtered = jobsToFilter.filter(job => {
            const matchesSearch = search === '' || (job.company?.company_profile?.company_name && job.company.company_profile.company_name.toLowerCase().includes(search.toLowerCase()));
            ...
            const matchesWorkLocation = selectedWorkLocation === '' || (selectedWorkLocation === 'remote' && job.is_remote) || (selectedWorkLocation === 'onsite' && !job.is_remote);
            const matchesJobTitle = selectedJobTitle === '' || (job.title && job.title.toLowerCase().includes(selectedJobTitle.toLowerCase()));
            const matchesGov = selectedGov === '' || selectedGov === 'all' || (job.company?.company_profile?.location && job.company.company_profile.location.toLowerCase().includes(selectedGov.toLowerCase()));
            return matchesSearch && matchesJobType && matchesExperience && matchesWorkLocation && matchesJobTitle && matchesGov;
        });
        setFilteredJobs(filtered);
        if (selectedJob && !filtered.some(job => job.id === selectedJob.id)) {
            setSelectedJob(filtered[0] || null);
        }
    }, [search, selectedJobType, selectedExperience, selectedWorkLocation, selectedJobTitle, selectedGov, jobs, selectedJob, showRecommended, recommendedJobs]);

    const handleGovChange = e => setSelectedGov(e.target.value);
    const handleJobTypeChange = e => setSelectedJobType(e.target.value);
    ...
    const keywordHandler = e => setSearch(e.target.value);
    ...
    const handleGetRecommendations = async () => {
        try {
            ...
            setError(null);
            const token = localStorage.getItem('token');
            if (!token) throw new Error('Please login to get recommendations');
            const skills = await fetchUserSkills(token);
            if (skills.length === 0) throw new Error('Please add skills to your profile to get recommendations');
            setUserSkills(skills);
            const recommendations = await getRecommendedJobs(skills, jobs);
            if (recommendations.length === 0) throw new Error('No recommendations found based on your current skills');
            setRecommendedJobs(recommendations);
            setShowRecommended(true);
            setFilteredJobs(recommendations);
            if (recommendations.length > 0) setSelectedJob(recommendations[0]);
        } catch (error) {
          ...
          <div className="rounded-xl bg-white/80 backdrop-blur-sm shadow-lg border border-white/30 p-6">
            ...
            <div className="flex flex-col lg:flex-row gap-6">
              ...
              <div className="w-full lg:w-2/5 h-[65vh] flex flex-col">
                  <div className="flex-shrink-0 mb-2 px-1">
                      <h3 className="text-sm font-semibold text-gray-600">
                          {showRecommended ? `Recommended Jobs (${filteredJobs.length})` : `Showing ${filteredJobs.length} of ${totalJobs} Jobs`}
                      </h3>
                  </div>
              ...
              <div className="w-full lg:w-3/5 h-[65vh] bg-white border border-gray-200 rounded-xl shadow-inner flex flex-col overflow-hidden">
                {selectedJob ? (
                    <>
                      {/* Redesigned Details Header */}
                      <div className="relative h-24 bg-gradient-to-br from-[#203947] to-[#901b20] flex-shrink-0 p-5 flex items-center">
                        <div className="w-14 h-14 bg-white/10 rounded-lg flex items-center justify-center mr-4">
                      ...
                      <div className="flex-1 overflow-y-auto p-5 space-y-5">
                        {showRecommended && selectedJob.matching_score && (
                           <div className="bg-blue-50/70 border border-blue-200 rounded-lg p-3">
                              <div className="flex items-center gap-3">
                                <Star className="w-6 h-6 text-blue-500 flex-shrink-0" fill="currentColor" />
                                <div>
                                  <h3 className="font-semibold text-blue-800">This is a Great Match for You!</h3>
                                  <p className="text-blue-700 text-sm">Matches {Math.round(selectedJob.matching_score)}% of your skills: {selectedJob.matched_skills?.join(', ')}</p>
                                </div>
                              </div>
                           </div>
                        )}
                        ...
                        <div>
                           <h3 className="font-semibold text-gray-800 text-sm mb-2">Job Description</h3>
                           <p className="text-sm text-gray-600 whitespace-pre-line bg-gray-50/70 p-4 rounded-lg border">{selectedJob.description}</p>
                        </div>
                        <div>
                           <h3 className="font-semibold text-gray-800 text-sm mb-2">Requirements</h3>
                           <p className="text-sm text-gray-600 whitespace-pre-line bg-gray-50/70 p-4 rounded-lg border">{selectedJob.requirements}</p>
                        </div>
                        <div>
                           <h3 className="font-semibold text-gray-800 text-sm mb-2">Required Skills</h3>
                           <div className="flex flex-wrap gap-2">
                              {selectedJob.skills?.map(skill => <span key={skill} className="bg-[#901b20]/10 text-[#901b20] text-xs font-medium px-3 py-1 rounded-full">{skill}</span>)}
                           </div>
                        </div>
                      </div>

                      <div className="flex-shrink-0 bg-gray-50/80 p-4 border-t">
                        <button className="w-full bg-gradient-to-r from-[#901b20] to-[#a83236] hover:shadow-lg text-white font-bold py-3 px-4 rounded-lg shadow-md transition-all transform hover:scale-[1.02]">
                          Apply Now
                        </button>
                      </div>
                    </>
                  ...
                  <div className="h-full flex items-center justify-center p-4 text-center">
                      <div>
                          ...
                          <p className="text-lg font-medium text-gray-600">Select a job to view details</p>
                          <p className="text-gray-400 text-sm">Your next career move is just a click away.</p>
                      </div>
                  </div>
                )}
              </div>
